...
cauchy_custom_pdf <- function(x, delta, p) {
...
for (loc in unique_locations) {
  ...
  cvm_test <- cvm.test(ip_subset$growthrate, cauchy_custom)
  ks_test <- ks.test(ip_subset$growthrate, cauchy_custom)
  ad_test <- ad.test(ip_subset$growthrate, cauchy_custom)
  ...
  new_row <- data.frame(
    LOCATION = loc,
    Opt_Param1 = opt_result$par[1],
    Opt_Param2 = opt_result$par[2],
