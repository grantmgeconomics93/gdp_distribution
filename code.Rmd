...
```
```{r}
# Define the function for Student's t-distribution PDF
pdf_student_t <- function(t, nu) {
  # Calculate the Gamma function terms
  gamma_term1 <- gamma((nu + 1) / 2)
  gamma_term2 <- gamma(nu / 2)
  
  # Calculate the constant in front of the formula
  constant <- gamma_term1 / (sqrt(nu * pi) * gamma_term2)
  
  # Calculate the main expression
  main_expr <- (1 + (t^2 / nu))^(-((nu + 1) / 2))
  
  # Calculate the full PDF value
  f_t <- constant * main_expr
...
for (loc in unique(ip$LOCATION)) {
  ...
  fit <- MASS::fitdistr(data_subset$growthrate, "pdf_student_t")
  
  # Extract parameters (modify this based on what specifically you want from fit)
  df_parameter <- fit$estimate[1]
  m_parameter <- fit$estimate[2]
  s_parameter <- fit$estimate[3]
  
  # Append the results for this location to the result_students_t dataframe
  result_students_t <- rbind(result_students_t, data.frame(
    Location = loc,
    df = df_parameter,
    m = m_parameter,
    s = s_parameter
...
# Remove row names for a cleaner look
row.names(result_students_t) <- NULL

# View the results
print(result_students_t)
