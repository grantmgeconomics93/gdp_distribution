...
library(MASS)
 ...
 objective_function <- function(params, data) {
  ...
  return(-sum(log_density, na.rm = TRUE))}
 optim(
...
```
```{r}
start_params <- c( d = 1, l = 0, v = 1)  # initial guesses
  fit <- optim(par = start_params, fn = objective_function
```
...
objective_function <- function(params, data) {
  ...
  if (any(is.nan(log_density))) {
    print(paste("NaN produced with parameters: d =", d, ", l =", l, ", v =", v))
  }
...
for (loc in unique(ip$LOCATION)) {
  ...
  start_params <- c( d = 1, l = 0, v = 1)  # initial guesses
  fit <- optim(par = start_params, fn = objective_function, data = data_subset$growthrate)
  ...
  # Append the results for this location to the result dataframe
