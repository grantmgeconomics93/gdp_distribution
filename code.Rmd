...
# Initialize the result_mle data frame before using it
# Example: Initialize result_mle with 10 rows (replace 10 with the number of rows you expect)
# Initialize result_mle with the correct number of rows based on unique locations
...
for (location_idx in 1:length(unique(ip$LOCATION))) {
  ...
  result_mle$se_b <- sqrt(diag(vcov(fit_gen))["b"])
  # Assume fit_gen is produced here, then calculate standard error and p-values
