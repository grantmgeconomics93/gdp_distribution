...
for (location in unique(ip$LOCATION)) {
  ...
  # Update the custom CDFs with the retrieved parameters
  custom_cdfgen <- function(x) {
    plaplace_modified(x, m = mu, a = a, b = b)
  }

  custom_cdfgenone <- function(x) {
    plaplace_modified(x, m = mu, a = a, b = 1)
  }

  # Perform the CVM tests
  cvm <- goftest::cvm.test(data$growthrate, custom_cdfgen)
...
for (location in unique(ip$LOCATION)) {
  ...
  cvm_test <- goftest::cvm.test(data$growthrate, "custom_cdfgen")
  ...
  parameter_tests_df <- rbind(new_tests_df, temp_new_tests)
}

# This will give you a new dataframe (new_tests_df) with the results of the CVM, KS, and AD tests.
